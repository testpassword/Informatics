...
filedata = urlopen('https://xml.meteoservice.ru/export/gismeteo/point/69.xml')  # открытие URL с XML погодой 
...
tree = ET.parse('C:/Users/Public/Documents/69.xml') # Получение XML в виде дерева 
...
for child in root:
    for ch in child.getchildren():
        if ch.tag == 'TOWN':
            town(ch)
        for forecast in ch.getchildren():
            if forecast.tag == 'FORECAST':
                day_of_the_week(forecast) 
            for f in forecast.getchildren():
